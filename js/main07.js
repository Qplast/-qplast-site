...
(function () {
  ...
  function imgForFamily(fam, vol) {
    if (fam === "dropper") return "../assets/dropper-product.png";
    if (fam === "qoil") return "../assets/qoil-product.jpg";
    if (fam === "sepanta") return "../assets/sepanta-product.jpg";
    if (fam === "aryne") return "../assets/pars-product.png";
    if (fam === "hamta") return "../assets/hamta-product.jpg";
    if (fam === "arya") return String(vol) === "30" ? "../assets/arya-30.jpg" : "../assets/arya-white.jpg";
    return "../assets/hamta-product.jpg";
  }
  ...
  function renderSpecs() {
    ...
    if (photoEl) {
      ...
      if (currentFamily === "dropper") {
        ...
        var activeNeck = String(currentNeck || f.bottles[0].neck || "");
  ...
  function updateFinder() {
    ...
    var ryCardImg = beautyCards.querySelector('article[data-family="arya"] .card__media img');
    if (ryCardImg) {
      ryCardImg.src = (neck === "18" && vol === "30") ? "../assets/arya-30.jpg" : "../assets/arya-white.jpg";
    }
    ...
    /* keep the open specs panel in sync with the finder neck/volume selection */
    var pnl = document.getElementById("specsPanel");
    if (pnl && !pnl.hidden) {
      if (neck !== "all" && neck) currentNeck = neck === "18" || neck === "20" ? Number(neck) : Number(neck);
      if (vol !== "all" && vol) currentVol = Number(vol);
      if (typeof renderSpecs === "function") renderSpecs();
    }
  }
  ...
  function showSpecs(fam) {
    ...
    else currentNeck = selNeck || null;

    /* read current volume selection from the finder dropdown */
    var selVol = null;
    if (finderVol) {
      var vActive = finderVol.querySelector(".finder__opt.active");
      if (vActive) {
        var vVal = vActive.getAttribute("data-vol");
        if (vVal !== "all") selVol = Number(vVal);
      }
    }
    if (selVol) currentVol = selVol;
